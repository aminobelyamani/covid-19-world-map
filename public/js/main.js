...
function socketListeners(socket) {
    socket.on('getFetchFromServer', data => {
        dataAPI = data.regionData;
        if (!usOn) { socket.emit('getVaccineData'); timeStamp.innerText = `Updated at ${formatTime()} EST`; }
    });
    ...
    socket.on('getUsVaccineData', data => {
        if (data) {
            fadeOut(_('chartLoader'));
            _('overlay').style.display = "none";
            usData = calcUsData(data);
            switchValue = 'vaccines';
            changeLegendColors('vaccines');
            const color = toggleSwitchCases('vaccines').color;
            currentData.style.backgroundColor = color;
            _('usStatsWrapper').scrollTop = 0;
            onUsMenuClick();
            addPopupListeners();
            addZoomTapListeners();
            addUsStateListeners();
...
function fetchAPI() {
    fetch('https://api.apify.com/v2/key-value-stores/SmuuI0oebnTWjRTUh/records/LATEST?disableRedirect=true')
        ...
        .then(data => {
            if (data) {
                dataAPI = data.regionData;
                if (!usOn) { getData(dataAPI); timeStamp.innerText = `Updated at ${formatTime()} EST`; }
            }
            else {
                socket.emit('getFetchFromServer');
            }
...
function getMinMax(data, property, usOn) {
    ...
    const sortData = (!usOn) ? countriesList : usData;
    showSortedList(sortData);
}
...
function showSortedList(data) {
    const color = toggleSwitchCases(switchValue).color;
    const list = (!usOn) ? _('worldList') : _('usStatsWrapper');
    let html = "";
    const dataSet = (!usOn) ? dataSVG : usSVG;
    sortList(data).forEach(item => {
        const flagId = dataSet.find(row => row.country === item.country).id;
...
function pathHover(e) {
    if (!usOn) { this.addEventListener('mouseup', onPathClick, false); }
...
function getPopupInfo(country) {
    ...
    let rankText = "";
    let rawTotalText = "";
    const flagId = (!usOn) ? dataSVG.find(data => data.country === country).id : usSVG.find(data => data.country === country).id;
...
function showCountryPopup(country, alpha2) {
    let usBtn = (alpha2.toLowerCase() === 'us') ? '<span class="us-btn" onMouseUp="showUsMap();">50 State View</span>' : '';
...
function onUsMenuClick(e) {
    prop = (e) ? this.getAttribute('data-us-prop') : 'percVacc';
    getMinMax(usData, prop, true);
    keyTitle.innerText = rawTotalSwitch(prop).title;
    const text = (e) ? this.innerText : 'Partially Vaccinated';
    _('usDropDownTitle').innerText = currentData.innerText = text;
    globalHelpTipHandler();
    if (window.innerWidth <= 768) { closeSideBar(); }
}
//GLOBAL LISTENERS
...
function removeZoomTapListeners() {
    ...
    if (is_touch_device) {
        ...
        //if (!usOn) { hammertime.off('doubletap').destroy(); presstime.off('doubletap').destroy(); }
    }
...
function onPinchZoom(e) {
    ...
    if ((currentMat.a - initialScale) < 0.1) {
        ...
        globalInstructions.classList.remove('transform-y-220');
        globalHelpTip.style.top = (globalInstructions.classList.length > 1 || usOn) ? "50px" : "70px";
    }
    else {
        if (window.innerWidth <= 768) {
            removeHeader();
...
function closeSideBar() {
    ...
    sideBarOpen = (!usOn) ? false : sideBarOpen;
}
function toggleSideBar() {
    if (sideBar.classList.length === 0) {
        ...
        toggle.style.opacity = 0;
        popup.style.display = "none";
        sideBarOpen = (!usOn) ? true : sideBarOpen;
    }
}
...
function globalHelpTipHandler() {
    ...
    if (is_touch_device && window.innerWidth <= 768) {
        globalHelpTip.style.top = (globalInstructions.classList.length > 1 || usOn) ? "50px" : "70px";
        console.log("tip" + globalHelpTip.style.top);
    }
}
...
function showPage() {
    ...
    toggle.style.opacity = (sideBarOpen === true) ? 0 : 1;
...
document.querySelectorAll('.menu-btns').forEach(btn => {
    btn.addEventListener('mouseup', function (e) {
        ...
        if (usOn) { removeUsStateListeners(); usOn = false; reassignProp(); }
        if (attr === "about") {
