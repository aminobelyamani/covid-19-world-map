function _(e){return document.getElementById(e)}var vh,pathCountries=[],countryCodes=[],dataSVG=[],usSVG=[],dataAPI=[],dataVacc=[],countriesList=[],usData=[],globalRangeList=[],prop="casesPerMil",currentTitle="Cases/Million";const rangeLimit=6;var switchValue="cases",mode="dark",onMaps=!0,dropDownOn=!1,chartOn=!1,usOn=!1,html=_("html"),header=_("header"),headerTitle=_("headerTitle"),loader=_("loader"),mobileNav=_("mobileNav"),hamburger=_("hamburger"),mapDiv=_("mapDiv"),worldMap=_("worldMap"),about=_("about"),globalInstructions=_("globalInstructions"),currentDataWrapper=_("currentDataWrapper"),globalHelpTip=_("globalHelpTip"),currentData=_("currentData"),toggleDark=_("toggleDark"),centerBtn=_("centerBtn"),keyBtn=_("keyBtn"),keys=_("keys"),legendHelpTip=_("legendHelpTip"),closeKeys=_("closeKeys"),keyTitle=_("keyTitle"),legendColors=_("legendColors"),toggle=_("toggleSideBar"),closeDash=_("closeDash"),sideBar=_("sideBar"),timeStamp=_("timeStamp"),switchWrapper=_("switchWrapper");const switchToggle=_("switchSVG");var is_touch_device,switchG=_("switchG"),switchCircle=_("switchCircle"),optionsDiv=_("optionsDiv"),dropDown=_("dropDown"),dropDownTitle=_("dropDownTitle"),casesMenu=_("casesMenu"),testsMenu=_("testsMenu"),deathsMenu=_("deathsMenu"),vaccMenu=_("vaccMenu"),statsWrapper=_("statsWrapper"),searchWrapper=_("searchWrapper"),searchIcon=_("searchIcon"),searchInput=_("searchInput"),closeSearch=_("closeSearch"),resultsWrapper=_("resultsWrapper"),countryPopup=_("countryPopup"),closePopup=_("closePopup"),popup=_("popup");let socket;var onLoad=!0;function onPageLoad(){statsWrapper.scrollTop=0,_("overlay").style.display="none",sideBar.style.visibility="visible",sideBar.className="",toggle.style.opacity=0,currentData.innerText=currentTitle,currentDataWrapper.style.visibility="visible",fadeOut(loader),fadeOut(_("overlayMsg")),is_touch_device="ontouchstart"in document.documentElement,is_touch_device?(switchToggle.addEventListener("touchend",onSwitchTap,!1),touchEvents(),addZoomTapListeners(),removeHover()):(switchToggle.addEventListener("mouseup",onSwitchClick,!1),addZoomTapListeners()),openLegend(),addPopupListeners(),showGlInstructions(),globalHelpTipHandler(),legendHelpTipHandler()}function showGlInstructions(){if(is_touch_device&&onMaps){globalInstructions.classList.remove("transform-y-260");const e=globalInstructions.querySelector("p");globalInstructions.style.display="block";const t=usOn?"state":"country";e.innerText=`Tap ${t} for info, press and hold for full ${t} profile.`}}function onDOMLoaded(){socket=io(),socketListeners(socket),socket.emit("getCountryCodes"),dataSVG=parseSVG(),pathStrokeHandler(),fetch("https://api.apify.com/v2/key-value-stores/SmuuI0oebnTWjRTUh/records/LATEST?disableRedirect=true").then(e=>{if(e.ok)return e.json()}).then(async e=>{e?(dataAPI=e.regionData,countriesList=await getData(dataAPI),countriesList=await calcData(countriesList),timeStamp.innerText=`Updated at ${formatTime()} EST`,execProp(),socket.emit("getVaccineData"),socket.emit("getUsData"),onPageLoad()):(socket.emit("getFetchFromServer"),onPageLoad())}),setInterval(()=>{fetchAPI()},3e5)}function socketListeners(e){e.on("getFetchFromServer",t=>{dataAPI=t.regionData,e.emit("getVaccineData"),e.emit("getUsData")}),e.on("getCountryCodes",e=>{countryCodes=e}),e.on("getLatestData",e=>{if(e.latest&&closePopup.getAttribute("data-country")===e.country){const t=getYestData(e),n=t.perc,a=t.percDeaths;n&&0!=n&&addPercNew(n),a&&0!=a&&addPercNew(a,!0)}}),e.on("getLatestWorldData",e=>{if(e.latest){const t=!("World"!==e.country),n=!("USA"!==e.country),a=getYestData(e,t,n),s=a.perc,r=a.percDeaths;"cases"===switchValue?s&&0!=s&&addPercNewWorld(s):"deaths"===switchValue&&r&&0!=r&&addPercNewWorld(r)}}),e.on("getVaccineData",async e=>{e&&(dataVacc=e,countriesList=await getData(dataAPI),countriesList=await calcData(countriesList),timeStamp.innerText=`Updated at ${formatTime()} EST`,execProp())}),e.on("getUsData",async e=>{e&&(onLoad&&(_("mapLoader")&&(fadeOut(_("mapLoader")),mapDiv.removeChild(_("mapLoader"))),onLoad=!1),usData=await calcUsData(e),usData=await calcData(usData),execProp())}),e.on("getCountryData",e=>{e?closePopup.getAttribute("data-alpha2").toLowerCase()===e.country.toLowerCase()&&onSocketChart(e):onSocketNoChart()}),e.on("getStateData",e=>{e?closePopup.getAttribute("data-country").toLowerCase()===e.country.toLowerCase()&&onSocketChart(e):onSocketNoChart()})}function onSocketChart(e){if(e.data){const t=_("chart");_("chartLoader")&&fadeOut(_("chartLoader")),t.style.minHeight="350px",t.style.height="calc(calc(100 * var(--vh)) - 60px)",dataHist=e.data,propArr=["new_cases_smoothed"],currentProp="new_cases_smoothed",propTitle=["Daily Cases"],chartArray=createChartArray(dataHist,currentProp),chartOn&&(removeChartListeners(),removeGlobalChartListeners()),makeChartDiv(),addGlobalChartListeners(),1<chartArray.length?(makeChart(),addChartListeners(),chartOn=!0):onNoChartData()}else onSocketNoChart()}function onSocketNoChart(){chartOn=!1;const e=_("chart");_("chartLoader")&&fadeOut(_("chartLoader")),e.innerHTML="<h2 class='chart-no-data yellow-test'>NO CHART DATA</h2>"}async function execProp(e){const t=usOn?usData:countriesList;buttonsHandler(prop),e||worldData(dataAPI),globalRangeList=await getMinMax(t,prop),makeLegend(globalRangeList),matchData(t,prop,globalRangeList),showSortedList(t)}function calcUsData(e){return new Promise(t=>{const n=[];e.forEach(e=>{e.partiallyVaccinated=Math.abs((e.peopleVaccinated||0)-(e.peopleFullyVaccinated||0)),n.push(e)}),t(n)})}function reassignProp(){execProp(),statsWrapper.scrollTop=0,globalHelpTipHandler();const e=usOn?"Search by State":"Search by Country";searchInput.setAttribute("placeholder",e)}function parseSVG(){pathCountries=zoomEl.querySelectorAll("path");const e=[];for(let t=0;t<pathCountries.length;t++)if(pathCountries[t].getAttribute("data-id")){let n=pathCountries[t].getAttribute("id").toLowerCase(),a=pathCountries[t].getAttribute("data-name");e.push({path:pathCountries[t],id:n,country:a})}return e}function fetchAPI(){fetch("https://api.apify.com/v2/key-value-stores/SmuuI0oebnTWjRTUh/records/LATEST?disableRedirect=true").then(e=>{if(e.ok)return e.json()}).then(async e=>{e?(dataAPI=e.regionData,socket.emit("getVaccineData"),socket.emit("getUsData")):socket.emit("getFetchFromServer")})}function getData(e,t){return new Promise(n=>{const a=[],s=t?usSVG:dataSVG;s.forEach(t=>{const n=e.findIndex(e=>e.country===t.country);if(-1!=n){e[n].alpha2=t.id;const s=dataVacc.findIndex(e=>e.country===t.id);e[n].totalVaccinations=-1==s?0:dataVacc[s].totalVaccinations,e[n].peopleVaccinated=-1==s?0:dataVacc[s].peopleVaccinated,e[n].peopleFullyVaccinated=-1==s?0:dataVacc[s].peopleFullyVaccinated,e[n].partiallyVaccinated=-1==s?0:Math.abs((dataVacc[s].peopleVaccinated||0)-(dataVacc[s].peopleFullyVaccinated||0)),e[n].vaccines=-1==s?"Not Reported":dataVacc[s].vaccines,a.push(e[n])}}),n(a)})}function calcData(e){return new Promise(t=>{const n=[],a=["totalDeaths","totalRecovered","activeCases"];var s=[];e.forEach(e=>{a.forEach((t,n)=>{s[n]=null==e[t]?null:roundVal(100*(e[t]/e.totalCases),1)});const t=100*(e.newCases/e.population),r=100*(e.newDeaths/e.population),o=100*(e.partiallyVaccinated/e.population),l=100*(e.peopleFullyVaccinated/e.population);e.newCasesPerMil=roundVal(1e4*t,1),e.newDeathsPerMil=roundVal(1e4*r,1),e.percDeaths=s[0],e.percRecovered=s[1],e.percActive=s[2],e.percCritical=null===e.seriousCritical?null:null!=e.activeCases&&0!=e.activeCases?roundVal(100*(e.seriousCritical/e.activeCases),1):0;e.percVacc=roundVal(o,1),e.percFullyVacc=roundVal(l,1),n.push(e)}),t(n)})}function totalOfProp(e){let t=0;return countriesList.forEach(n=>{t+=n[e]}),t}function worldData(e){const t=usOn?"USA":"World",n=e.find(e=>e.country===t),a=usOn?n.population:totalOfProp("population"),s=usOn?n.newCasesPerMil:roundVal(1e6*(n.newCases/a),1),r=usOn?n.newDeathsPerMil:roundVal(1e6*(n.newDeaths/a),1),o=usOn?n.totalTests:totalOfProp("totalTests"),l=usOn?n.partiallyVaccinated:totalOfProp("partiallyVaccinated"),c=usOn?n.peopleFullyVaccinated:totalOfProp("peopleFullyVaccinated"),d=usOn?n.totalVaccinations:totalOfProp("totalVaccinations");n.totalTests=o,n.totalPop=a,n.totalPartialVacc=l,n.totalPplFlVacc=c,n.totalVacc=d;const p=roundVal(100*(n.totalRecovered/n.totalCases),1),u=roundVal(100*(n.activeCases/n.totalCases),1),h=roundVal(100*(n.totalDeaths/n.totalCases),1),y=roundVal(100*(n.seriousCritical/n.activeCases),1),m=roundVal(100*(n.totalPartialVacc/n.totalPop),1),f=roundVal(100*(n.totalPplFlVacc/n.totalPop),1);let g=_("worldStats");const v=usOn?"USA":"World",b=usOn?"US":"Global";"cases"===switchValue?(g.innerHTML=`
            ...
            </div>`,socket.emit("getLatestWorldData",v))}function buttonsHandler(e){const t=toggleSwitchCases(switchValue).btns,n=document.querySelectorAll(t),a=toggleSwitchCases(switchValue).color;for(let t=0;t<n.length;t++)n[t].dataset.prop===e?(n[t].style.backgroundColor=a,n[t].style.color="#000"):(n[t].style.backgroundColor=bckColorLDM,n[t].style.color=colorLDM);currentData.innerText=dropDownTitle.innerText=currentTitle,keyTitle.innerText=rawTotalSwitch(e).title}function getMinMax(e,t){return new Promise(n=>{const a=mapData(e,t);var s="casesPerMil"==t?a[1]:Math.max.apply(null,a),r=1<Math.min.apply(null,a.filter(Boolean))?1:Math.min.apply(null,a.filter(Boolean));const o=r.countDecimals();r=1>r?1/Math.pow(10,o):r;const l="percRecovered"===t||"percActive"===t||"percCritical"===t||"percVacc"===t||"percFullyVacc"===t?getPercRangeList(s,r):getRangeList(s,r);n(l)})}function mapData(e,t){const n=e.map(e=>(e[t]=0>e[t]?0:e[t],e[t]));return n.sort((e,t)=>e<t?1:-1)}function getPercRangeList(e,t){if(0===e)return[];const n=[],a=1>t?5:6;e=1<=Math.ceil(e/a)?Math.ceil(e/a):1;let s=t;for(let a=0;a<rangeLimit;a++)n.push(s),s=1>s?1:1===s&&2<=e?s+(e-1):s+e;return n}function getRangeList(e,t){const n=[];for(var a,s=0;s<rangeLimit;)a=0|Math.log(e)*Math.LOG10E+1,e=3>=a&&1===s?Math.floor(e):Math.floor(e/Math.pow(10,a-1))*Math.pow(10,a-1),1<=e?(5===s&&(e=t),n.push(e)):1>t&&(n.push(t),s=rangeLimit),e=5>=a&&0===s||"percDeaths"===prop?e/2:e==Math.ceil((e+1)/Math.pow(10,a+1))*Math.pow(10,a-1)?e/10:Math.ceil((e+1)/Math.pow(10,a+1))*Math.pow(10,a-1),s++;return n.reverse(),n}function changeLegendColors(e){const t=legendColors.querySelectorAll(".color"),n=toggleSwitchCases(e).colors.reverse();for(let a=0;a<t.length-1;a++)t[a].style.backgroundColor=n[a]}function makeLegend(e){const t=keys.querySelectorAll(".data-range"),n=legendColors.querySelectorAll(".color");for(let n=0;n<t.length;n++)t[n].classList.add("no-display");for(let t=0;t<n.length;t++)n[t].classList.add("no-display"),n[t].style.borderTopRightRadius="0px",n[t].style.borderTopLeftRadius="0px",n[t].style.borderBottomRightRadius="0px";for(let a=1;3>a;a++)t[t.length-a].classList.remove("no-display"),n[n.length-a].classList.remove("no-display");onLegendResize(e)}function onLegendResize(e){const t=keys.querySelectorAll(".data-range"),n=legendColors.querySelectorAll(".color");t[t.length-1].innerText=768>=window.innerWidth?"No Data":"No Reported Data",0===e.length&&(n[n.length-2].style.borderTopRightRadius="3px",768>=window.innerWidth?n[n.length-2].style.borderBottomRightRadius="3px":n[n.length-2].style.borderTopLeftRadius="3px");for(let a=0;a<e.length;a++){const s=t.length-(3+a),r=e[a].countDecimals(),o=1>=e[a]?0:"newCasesPerMil"===prop||"percRecovered"===prop||"percActive"===prop||"percCritical"===prop||"newDeathsPerMil"===prop||"percDeaths"===prop||"percVacc"===prop||"percFullyVacc"===prop?.1:1,l=1>e[a]&&1===e[a+1]?1/Math.pow(10,r):0;t[s].classList.remove("no-display"),n[s].classList.remove("no-display"),a==e.length-1?(n[s].style.borderTopRightRadius="3px",t[s].firstElementChild.innerText=`> ${e[a].commaSplit()}`,768>=window.innerWidth?n[s].style.borderBottomRightRadius="3px":n[s].style.borderTopLeftRadius="3px"):t[s].firstElementChild.innerText=768>=window.innerWidth?`${e[a].commaSplit()}`:`${(e[a]+o).commaSplit()} – ${(e[a+1]-l).commaSplit()}`}const a=n[n.length-2].getBoundingClientRect().width,s=768>=window.innerWidth?-(a/2+.5)+"px":"0px";for(let n=0;n<t.length;n++)t[n].style.width=768>=window.innerWidth?`${a}px`:"auto",n!=t.length-1&&t[n].style.setProperty("--trans",s)}function matchData(e,t,n){for(let a=0;a<pathCountries.length;a++)if(pathCountries[a].getAttribute("data-name")){let s=pathCountries[a].getAttribute("data-name");const r=e.findIndex(e=>e.country===s);if(-1!=r){const s=e[r][t];pathCountries[a].style.fill=0<=s&&null!=s?val2color(s,n):"#9c9c9c"}else pathCountries[a].style.fill="#9c9c9c"}}function val2color(e,t){const n=toggleSwitchCases(switchValue).colors;let a="";return 0===t.length||0===e?n[0]:(t.forEach((s,r)=>{0===r?e>=s&&e<=t[r+1]&&(a=n[r+1]):r===t.length-1?e>t[t.length-1]&&(a=n[r+1]):e>s&&e<=t[r+1]&&(a=n[r+1])}),a)}function sortList(e,t){t||(t=prop);var n=[];e.forEach(e=>{null!=e[t]&&(e[t]=0>e[t]?0:e[t],n.push(e))}),n.sort((e,n)=>e[t]<n[t]?1:-1);let a=0;return n.forEach((e,s)=>{a=0<s?n[s][t]===n[s-1][t]?a:a+1:a+1,e.rank=a}),n}function showSortedList(e){const t=toggleSwitchCases(switchValue).color,n=_("worldList");let a="";const s=usOn?usSVG:dataSVG;sortList(e).forEach(e=>{const t=s.find(t=>t.country===e.country).id,n="ic"===t?`images/ic.png`:"us-dc"===t?`images/us-dc.png`:`https://flagcdn.com/60x45/${t}.png`;a+=`
        ...
        ${t}
        ${n}
        ${a}`}function pathMove(t){if(mouseMove)return void(popup.style.display="none");var n,a,s;if(t=t||window.event,is_touch_device?(t.pageX=t.center.x,t.pageY=t.center.y):null==t.pageX&&null!=t.clientX&&(n=t.target&&t.target.ownerDocument||document,a=n.documentElement,s=n.body,t.pageX=t.clientX+(a&&a.scrollLeft||s&&s.scrollLeft||0)-(a&&a.clientLeft||s&&s.clientLeft||0),t.pageY=t.clientY+(a&&a.scrollTop||s&&s.scrollTop||0)-(a&&a.clientTop||s&&s.clientTop||0)),t.target.parentNode===zoomEl){popup.style.display="block";const e=t.pageX+popup.offsetWidth+10,n=t.pageY+popup.offsetHeight,a=e>window.innerWidth?`${Math.max(t.pageX-popup.offsetWidth-5,0)}px`:`${t.pageX+5}px`,s=n>window.innerHeight?`${Math.max(t.pageY-popup.offsetHeight-5,0)}px`:`${t.pageY+5}px`;popup.style.msTransform=`translate(${a}, ${s})`,popup.style.webkitTransform=`translate(${a}, ${s})`,popup.style.MozTransform=`translate(${a}, ${s})`,popup.style.OTransform=`translate(${a}, ${s})`,popup.style.transform=`translate(${a}, ${s})`}else popup.style.display="none"}function pathNoHover(){this.removeEventListener("mouseup",onPathClick),this.removeEventListener("mouseout",pathNoHover)}function resultsTransform(){resultsWrapper.style.width=`${getOffsets(searchWrapper).width}px`,resultsWrapper.style.msTransform=`translateX(${getOffsets(searchWrapper).left}px)`,resultsWrapper.style.webkitTransform=`translateX(${getOffsets(searchWrapper).left}px)`,resultsWrapper.style.MozTransform=`translateX(${getOffsets(searchWrapper).left}px)`,resultsWrapper.style.OTransform=`translateX(${getOffsets(searchWrapper).left}px)`,resultsWrapper.style.transform=`translateX(${getOffsets(searchWrapper).left}px)`}async function showCountryPopup(e,t){onMaps=!1;let n="us"===t.toLowerCase()?"<button class=\"us-btn\" onMouseUp=\"onMenuBtn();\">50 STATE MAP</button>":"";countryPopup.scrollTo(0,0),countryPopup.classList.remove("transform"),closePopup.style.visibility="visible",closePopup.style.marginLeft="-5px",closePopup.setAttribute("data-alpha2",t),closePopup.setAttribute("data-country",e),setTimeout(()=>{resultsTransform()},300);const a=usOn?["newCasesPerMil","newDeathsPerMil","percDeaths","percRecovered","percActive","testsPerMil","percVacc","percFullyVacc","totalVaccinations"]:["newCasesPerMil","newDeathsPerMil","percDeaths","percRecovered","percActive","percCritical","testsPerMil","percVacc","percFullyVacc","totalVaccinations"],s=usOn?["Daily New Cases","Daily New Deaths","Case Fatality Rate","Recovered","Active","Tests","Partially Vaccinated","Fully Vaccinated","Vaccinations"]:["Daily New Cases","Daily New Deaths","Case Fatality Rate","Recovered","Active","Critical","Tests","Partially Vaccinated","Fully Vaccinated","Vaccinations"],r=usOn?usSVG:dataSVG,o=r.find(t=>t.country===e).id,l="ic"===o?`images/ic.png`:"us-dc"===o?`images/us-dc.png`:`https://flagcdn.com/h240/${o}.png`;let c,d=[];const u=usOn?usData:countriesList;["casesPerMil","deathsPerMil","population"].forEach(t=>{const n=sortList(u,t);c=n.find(t=>t.country===e),d.push(c.rank)});var h="";a.forEach((e,t)=>{const n=rawTotalSwitch(e).title,a=rawTotalSwitch(e).totalProp,r=rawTotalSwitch(e).colorClass,o="totalVaccinations"!==e||usOn?"":"stats-column-flex-100",l=dropDownSwitch(e);h+=`
            <div class='stats-column-flex ${o}'>
                <div class="help-tip-stats help-tip"><p>${l}</p></div>
                <p class='prop-title ${r}'>${s[t]}</p>
            ...
            <div class='flex-stats-container'>
                ${h}
                <div id="chart"></div>
            </div>`;const y=await dynLoader("chartLoader");y?(_("chart").appendChild(y),fadeIn(_("chartLoader"))):alert("Network Error: Check your internet connection");const m={alpha2:t,country:e,usOn:usOn};socket.emit("getLatestData",m),usOn?socket.emit("getStateData",e):socket.emit("getCountryData",t),setTimeout(()=>{addHeader()},300),countryPopup.addEventListener("scroll",onCountryPopupScroll,!1)}function onCountryPopupScroll(){768>=window.innerWidth&&addHeader()}function getYestData(e,t,n){const a=!usOn||n?countriesList:usData,s=[],r=t?dataAPI.find(e=>"World"===e.country):sortList(a,"newCases").find(t=>t.country===e.country);return["newCases","newDeaths"].forEach(t=>{const n=0<e.latest[t]?e.latest[t]:1;let a=100*((r[t]-(e.latest[t]||0))/n);a=roundVal(a,2),s.push(a)}),{perc:s[0],percDeaths:s[1]}}function addPercNew(e,t){const n=t?2:1,a=countryPopup.querySelector(`.flex-stats-container .stats-column-flex:nth-child(${n})`);if(5===a.childElementCount){const t=document.createElement("div");t.setAttribute("class","flex-stat"),t.setAttribute("style","opacity:0;");const n=1===Math.sign(e)?"dark"===mode?"#f6584C":"#B13507":"dark"===mode?"#6dff71":"#209222",s=-1===Math.sign(e)?"\u25BC":"\u25B2",r=1===Math.sign(e)?"+":"";t.innerHTML=`
        <p class='stats' style='color: ${n};font-size:1em;'>${s}</p>
        <p class='stats-titles' style='color: ${n};'>${r+e}%</p>`,a.appendChild(t),fadeIn(t,!0)}}function addPercNewWorld(e){const t=worldStats.querySelectorAll(`.worldStats-flex`)[0];if(t){const n=t.querySelector(".flex-stat");if(n){n.style.opacity=0;const t=1===Math.sign(e)?"#f6584C":"#6dff71",a=-1===Math.sign(e)?"\u25BC":"\u25B2",s=1===Math.sign(e)?"+":"";n.innerHTML=`
            <p class='stats' style='color: ${t}; font-size:1em;'>${a}</p>
            <p class='stats-titles' style='color: ${t};'>${s+e}%</p>`,fadeIn(n,!0)}}}function getPiePerc(e,t,n){const a=n?25:30,s=n?118:141,r=n?157:188,o=n?2:5,l=n?"width:60px; height:70px; margin-top:0;":"",c="totalRecovered"===t||"percRecovered"===t||"percVacc"===t||"percFullyVacc"===t?"dark"===mode||n?"#f6584C":"#B13507":"dark"===mode||n?"#6dff71":"#209222",d="#f6584C"===c||"#B13507"===c?"dark"===mode||n?"#6dff71":"#209222":"dark"===mode||n?"#f6584C":"#B13507",p="percCritical"===t||"seriousCritical"===t?"% of Active":"percVacc"===t||"percFullyVacc"===t?"% of Pop":"% of Cases",u=n?"none":"block";let h=`
    <div class="pie-wrapper" style='${l}' title='${n?p:""}'>
        <svg class="circle">
            <circle r="${a}" cx="50%" cy="50%" stroke="${c}" fill="none" stroke-width="${o}" stroke-linecap="round" stroke-dasharray="${s}, ${r}" stroke-opacity="0.2"></circle>
            <circle r="${a}" cx="50%" cy="50%" stroke="${d}" fill="none" stroke-width="${o}" stroke-linecap="round" stroke-dasharray="${e/100*s}, 188"></circle>
        </svg>
        <div class="circle-info circle-perc" style="color:${d};">${e}</div>
        <div class="circle-info" style="color:${d}; display:${u};">${p}</div>
    </div>`;return h}function getRecord(e,t,n,a){const s=usOn?usData:countriesList,r=sortList(s,t),o=r.find(t=>t.country===e);let l="",c="",d="",p="";if(o){c=`
            ...
            </div> `;let r;return"percDeaths"===t?r=s.find(t=>t.country===e).percDeaths:"percRecovered"===t?r=s.find(t=>t.country===e).percRecovered:"percActive"===t?r=s.find(t=>t.country===e).percActive:"percCritical"===t?r=s.find(t=>t.country===e).percCritical:"percVacc"===t?r=s.find(t=>t.country===e).percVacc:"percFullyVacc"===t?r=s.find(t=>t.country===e).percFullyVacc:void 0,p=0<=r&&null!=r?getPiePerc(r,t):`<div class='flex-stat'>
        ...
        <polyline class='polyline' stroke='${u}' points='${l}'></polyline>`;r.innerHTML=y,t.appendChild(r)}function propSwitch(e){let t;return"new_cases_smoothed"===e?t="dark"===mode?"#54cbf2":"#038ebc":"new_deaths_smoothed"===e?t="dark"===mode?"#f44336":"#B13507":"new_vaccinations_smoothed"===e?t="dark"===mode?"#4cf6af":"#33a976":"stringency_index"===e?t="dark"===mode?"#ffc82a":"#967000":void 0,t}function appendHoverG(){const e=_("chart"),t=_("svgChart"),n=document.createElementNS("http://www.w3.org/2000/svg","g");n.setAttribute("class","hover-info"),t.appendChild(n);const a=propSwitch(currentProp),s="dark"===mode?"#ffc82a":"#967000";let r=`<line class='hover-line' x1="0" x2="0" y1="0" y2="${getBoundValues().yBound}" stroke-width='1px'></line>`;r+=`<circle class='hover-dot' cx="0" cy="0" r="5" fill='${a}' style='visibility:hidden;'></circle>`,r+=`<circle class='hover-dot' cx="0" cy="0" r="5" fill='${s}' style='visibility:hidden;'></circle>`,n.innerHTML=r;const o=document.createElement("div");o.setAttribute("class","chart-popup"),e.appendChild(o)}function makeChartDiv(){const e=_("chart"),t=usOn?"":"<button type=\"button\" class=\"chart-btns\" data-chartprop=\"stringency_index\">Stringency Index</button>";e.innerHTML=`
